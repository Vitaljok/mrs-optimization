...
@XmlRootElement
@Entity
public class GreenhouseSprayingMission extends GreenhouseMission {       
    ...
    public void setSprayingTime(Double sprayingTime) {
    ...
    public Double getAgentPerformance(Agent agent) {
        ...
        if (compMap.containsKey(Const.familyPlatform)
                ...
                && compMap.containsKey(Const.familySpray)) {
            ...
            Double movingTime = ((this.intervalPlants - approachDistance) / this.workDensity) / robotMovingSpeed;

            // time for positioning near plant
            Double positioningTime = approachDistance / (this.requiredTolerance * sampleRate);
                                    
            // work units per sec
            return 1 / (movingTime + positioningTime + sprayingTime);
        } else {
            return 0d;
        }
    }
    ...
    public Double getAmountOfWork() {
        ...
        return 1d * this.numOfWorkCicles * this.numOfPlants * this.numOfRows;
    }

    public GreenhouseSprayingMission(Integer numOfRows, Integer numOfPlants, Integer numOfWorkCicles) {
        super(numOfRows, numOfPlants, numOfWorkCicles);
    }   

    public GreenhouseSprayingMission() {
        super();
    }
}
